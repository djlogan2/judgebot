...
public class PolyglotBook {
	...
	//private HashMap<Long, ArrayList<move>> book;
	private RandomAccessFile inputFile;
	private long number_of_entries;
	public PolyglotBook(String bookFileName) {
		try {
			inputFile = new RandomAccessFile(bookFileName, "r");
			number_of_entries = inputFile.length() / 16;
	...
	public ArrayList<entry> getBookMoves(Fen fen) {
		return findKey(hash(fen));
	}

	protected ArrayList<entry> findKey(long key) {
		long last = -1;
		long top = 0;
		long bottom = number_of_entries;
		long middle;
		long key_high = (key >> 63) * -1;
		long key_low  = (key & 0x7FFFFFFFFFFFFFFFL);
		ArrayList<entry> entrylist = new ArrayList<entry>();
		
		while(true) {
			middle = (bottom - top) / 2 + top;
			entry entry = getEntry(middle);
			...
			if(entry.key == key) {
				...
				entry = getEntry(++middle);
				while(middle != number_of_entries && entry.key == key) {
					entrylist.add(entry);
					entry = getEntry(++middle);
				}
			...
			long entry_high = (entry.key >> 63) * -1;
			long entry_low  = (entry.key & 0x7FFFFFFFFFFFFFFFL);
			
			last = middle;
			if(entry_high < key_high || (entry_high == key_high && entry_low < key_low)) {
				top = middle;
			} else { // entry.key > key
				bottom = middle;
			}
	...
	private entry getEntry(long idx) {
		entry e = new entry();
		try {
			inputFile.seek(16 * idx);
			e.key = inputFile.readLong();
			e.move = inputFile.readShort();
			e.weight = inputFile.readShort();
			e.learn = inputFile.read();
			return e;
		} catch (IOException e1) {
			inputFile = null;
			return null;
	...
	protected long hash(NormalChessGame game) {
		try {
			return hash(game.getFEN());
		} catch (Exception e) {
			e.printStackTrace();
			return 0;
		}
    ...
    protected long hash(Fen fen) {
    	long hash = 0;
        for(int rank = 0 ; rank < 8 ; rank++) {
            for(int file = 0 ; file < 8 ; file++) {
            	ChessPiece p = fen.board()[rank][file];
            	if(p.piece() != ChessPieceEnum.NONE)
            		hash ^= getPieceHash(rank, file, p);
            }
        }
        
        if(fen.whiteCanCastle()[Fen.KINGSIDE]) hash ^= RANDOM64[RANDOMCASTLE];
        if(fen.whiteCanCastle()[Fen.QUEENSIDE]) hash ^= RANDOM64[RANDOMCASTLE+1];
        if(fen.blackCanCastle()[Fen.KINGSIDE]) hash ^= RANDOM64[RANDOMCASTLE+2];
        if(fen.blackCanCastle()[Fen.QUEENSIDE]) hash ^= RANDOM64[RANDOMCASTLE+3];
        
        if(fen.enpassant() != null) {
        	hash ^= RANDOM64[RANDOMENPASSANT+fen.enpassant().file];
        }
        
        if(fen.toMove() == Color.WHITE)
        	hash ^= RANDOM64[RANDOMTURN];
        
        return hash;
    }


	private long getPieceHash(int rank, int file, ChessPiece p) {
    	int v = 0;
    	switch(p.piece()) {
    	case PAWN:   v =  0; break;
    	case KNIGHT: v =  2; break;
    	case BISHOP: v =  4; break;
    	case ROOK:   v =  6; break;
    	case QUEEN:  v =  8; break;
    ...
    private static int RANDOMCASTLE    = 768;
    private static int RANDOMENPASSANT = 772;
    private static int RANDOMTURN      = 780;
    private static long RANDOM64[] = new long[] {
    ...
    public class entry implements Comparable<entry> {
    	...
    	public Move getMove() {
    		try {
				return new Move(shortSmith(), true);
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			}
    	...
    	private String shortSmith() {
			int to_file   = (move      ) & 7;
			int to_row    = (move >>  3) & 7;
			int from_file = (move >>  6) & 7;
			int from_row  = (move >>  9) & 7;
			int promote   = (move >> 12) & 7;

			String promotePiece = "";
			switch(promote) {
			case 0: break;
			case 1: promotePiece = "n"; break;
			case 2: promotePiece = "b"; break;
			case 3: promotePiece = "r"; break;
			case 4: promotePiece = "q"; break;
			}
			
			Square fromSquare = new Square(from_row, from_file);
			Square toSquare   = new Square(to_row, to_file);
			//
			// Fix the weird castling
			//
			if(fromSquare.equals("e1") && toSquare.equals("h1")) toSquare = new Square("f1");
			if(fromSquare.equals("e1") && toSquare.equals("a1")) toSquare = new Square("c1");
			if(fromSquare.equals("e8") && toSquare.equals("h8")) toSquare = new Square("f8");
			if(fromSquare.equals("e8") && toSquare.equals("a8")) toSquare = new Square("c8");
			String shortSmith = fromSquare.toString() + toSquare.toString() + promotePiece;
			return shortSmith;
    	}
    	...
    	{
    		return String.format("Key: %d, Move: %s, Weight: %d, Learn: %d", key, getMove(), weight, learn);
        }
